...
public final class CnnMnist {
  ...
  public static Graph build(String optimizerName) {
    ...
    // Optimizer
    Optimizer optimizer = switch (lcOptimizerName) {
        case "adadelta" -> new AdaDelta(graph, 1f, 0.95f, 1e-8f);
        case "adagradda" -> new AdaGradDA(graph, 0.01f);
        case "adagrad" -> new AdaGrad(graph, 0.01f);
        case "adam" -> new Adam(graph, 0.001f, 0.9f, 0.999f, 1e-8f);
        case "sgd" -> new GradientDescent(graph, 0.01f);
        case "momentum" -> new Momentum(graph, 0.01f, 0.9f, false);
        case "rmsprop" -> new RMSProp(graph, 0.01f, 0.9f, 0.0f, 1e-10f, false);
  ...
  public static void test(Session session, int minibatchSize, MnistDataset dataset) {
    ...
    for (ImageBatch trainingBatch : dataset.testBatches(minibatchSize)) {
      try (TUint8 transformedInput = TUint8.tensorOf(trainingBatch.images());
          var result = session.runner().feed(INPUT_NAME, transformedInput).fetch(OUTPUT_NAME).run()) {
  ...
  public static void main(String[] args) {
    ...
    try (Graph graph = build(optimizerName);
